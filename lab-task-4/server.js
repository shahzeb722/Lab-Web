...
const JWT_SECRET = process.env.JWT_SECRET || 'labweb_jwt_secret';
...
const buildProductQuery = (queryParams) => {
  ...
  const minPriceValue = Number.parseFloat(queryParams.minPrice);
  const maxPriceValue = Number.parseFloat(queryParams.maxPrice);
  ...
  const sort = (queryParams.sort || 'name_asc').trim();
  ...
  return {
    ...
    sort,
    mongoQuery,
    sortBy: sortOptions[sort] || sortOptions.name_asc
  };
};
...
app.get('/api/v1/products', async (req, res) => {
  const page = Math.max(parseInt(req.query.page, 10) || 1, 1);
  const limit = 8;
  const { mongoQuery, sortBy } = buildProductQuery(req.query);
...
app.get('/api/v1/user/profile', verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.jwtUser.user_id).select('name email role').lean();
    if (!user) {
